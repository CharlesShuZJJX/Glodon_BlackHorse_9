#include "com_demo_opencv_UtilsToService.h"
...
public:
    ...
    static int findConnectedDomain(Mat& srcImg, vector<vector<Point>>& connectedDomains, int area, int WHRatio);// 检测连通域，并删除不符合条件的连通域[CSDN源码]
    ...
    static int setEdgeBlack(Mat& src); //将边缘涂黑[07-09]
...
int CvGeFunctions::findConnectedDomain(Mat& srcImg, vector<vector<Point>>& connectedDomains, int area, int WHRatio)
{
...
void CvGeFunctions::getSkeletonCurve(Mat& srcImg, vector<vector<Point>>& vecSkeletonCurves)
{
    ...
    findContours(srcImg, vecPts, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_NONE);

    for (int i = 0; i < vecPts.size(); ++i)
    {
        vecSkeletonCurves.push_back(vecPts[i]);
    }
...
int CvGeFunctions::setEdgeBlack(Mat& src)
{
    int nRow = src.rows;
    int nCol = src.cols;
    for (int i = 0; i < nRow; ++i)
...
ErrorStatus CvProgramFunctions::getConnectedDomain(vector<vector<Point>>& connectedDomains, Mat& orgImg, Mat& resImg)
{
    ...
    cvtColor(orgImg, resImg, COLOR_RGBA2GRAY, 1); // res灰度图
    ...
    Canny(orgImg, resImg, cannyMin, cannyMax, 3); // 边缘检测,覆盖之前的resImg
    ...
    vector<Vec4i>hierarchy;
    vector<vector<Point>> vecPts;
    findContours(resImg, vecPts, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_NONE);
    //vector<vector<Point>> vecPts;
    connectedDomains.resize(vecPts.size());
    for (int i = 0; i < vecPts.size(); ++i)
    {
        for (int j = 0; j < resImg.rows; ++j)
        {
            for (int k = 0; k < resImg.cols; ++k)
            {
                Point ptToJudge(j, k);
                if (pointPolygonTest(vecPts[i], ptToJudge, false) == true)
                {
                    connectedDomains[i].push_back(ptToJudge);
                }
            }
        }
    }
    imwrite(GLOBAL_OUTPUT_DIR + "res.jpg", resImg);
...
//    void *pixels;
////    Mat* pMatSrc=(Mat*)mSrcAddr;
////    Mat* pMatDst=(Mat*)mDstAddr;
////    Mat* temp;
//
...
//        int cannyMax = (avgGray) > 255 ? 255 : (avgGray);
//        int cannyMin = cannyMax / 5; // 上下阈值之比为5:1 [ssr 07-09]
//        Canny(resImg, resImg, cannyMin, cannyMax, 3);
//        cvtColor(resImg, orgImg, COLOR_GRAY2RGBA, 1);
////        int errorStatue = CvProgramFunctions::getConnectedDomain(connectedDomains, *pMatSrc, *temp);
////        CvProgramFunctions::showCracks(connectedDomains, *pMatSrc, *pMatDst);
//
////        int errorStatue = CvProgramFunctions::getConnectedDomain(connectedDomains, orgImg, resImg);
...
JNICALL Java_com_demo_opencv_NativeActivity_getEdge
    ...
    if (info.format == ANDROID_BITMAP_FORMAT_RGBA_8888) {
        Mat temp(info.height, info.width, CV_8UC4, pixels);
        Mat gray;
        CvGeFunctions::addContrast(temp);
        cvtColor(temp, gray, COLOR_RGBA2GRAY);
//        int avgGray = CvGeFunctions::getCannyThresholdUp(gray);
//        int cannyMax = (avgGray) > 255 ? 255 : (avgGray);
//        int cannyMin = cannyMax / 5; // 上下阈值之比为5:1 [ssr 07-09]
        Canny(gray, gray, 60, 90);
        Mat kernel = getStructuringElement(MORPH_ELLIPSE, Size(9, 9));
        morphologyEx(gray, gray, MORPH_CLOSE, kernel, Point(-1, -1), 3);
        Mat kernel_1 = getStructuringElement(MORPH_ELLIPSE, Size(5, 5));
        ...
        vector<vector<Point>> vecPts;
        vector<vector<Point>> connectedDomains;
        findContours(gray, vecPts, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_NONE);
        Mat temp_1;
        cvtColor(temp, temp_1, COLOR_RGBA2BGR);
        drawContours(temp_1,vecPts,-1,(255,0,0),3);
        cvtColor(temp_1, temp, COLOR_BGR2RGBA);
    } else {
        Mat temp(info.height, info.width, CV_8UC2, pixels);
        Mat gray;
        cvtColor(temp, gray, COLOR_RGB2GRAY);
...
//        int cannyMin = cannyMax / 5; // 上下阈值之比为5:1 [ssr 07-09]
    ...
    AndroidBitmap_unlockPixels(env, bitmap);
    jdouble buf[]={7.8124, 0.1664, 1.0269};
    int len = 3;
    jdoubleArray arr = env -> NewDoubleArray(len);
    env -> SetDoubleArrayRegion(arr, 0, len, buf);
    return arr;
}
